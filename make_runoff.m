function [out_river_ori, output_indomain_last,Lsrc_,Tsrc_,number_of_river] = make_runoff(input_def_dir, river_orientation, input_indomain_last,YEAR,MONTH);



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  MODIFIED by Md Nazmus Sanib Chowdhury on September 16, 2026.
%
%
%  Reference: 
...
%
%
%  The code was modified to generate the runoff forcing files aligning
%  the temperature and salinity data from 
%  "croco_clm_mercator_Y????M??.nc" files for a user defined range of
%  years and months. A "croco_clm_mercator_Y????M??" is created during
%  the generation of surface ocean forcing files from GLORYS MERCATOR
%  through executing the croco_tools/Oforc_OGCM/make_OGCM_mercator.m 
%  file in the MATLAB editor while, following the instruction provided 
%  in the croco_tools/Oforc_OGCM/readme.md MATLAB editor. The "????" 
%  and "??" after "Y" and "M" in the "croco_clm_mercator_Y????M??"
%  indicate year and month respectively. In this function, each year 
%  and month are provided externally as function arguments. The river
%  runoff forcing files are then created through calling this function 
%  (i.e., make_runoff) through the for loops of years and months. The
%  was developed in such a way that, the user not need to select the
%  river and the river orientation of every time of calling this 
%  function. It is sufficient to call this function for the first 
%  month of first year of the simulation's temporal coverage only.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%% The original block of heading for this script  %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if ( gridlevel == 0 ) 
    ...
    rivname_ = sprintf('croco_runoff_DAI_Y%dM%s.nc',YEAR,MONTH);
    rivname =  [CROCO_files_dir,rivname_];
    clmname_ = sprintf('croco_clm_mercator_Y%dM%s.nc',YEAR,MONTH);
    clmname = [CROCO_files_dir,clmname_];    % <- climato file for runoff
else 
    % # -> Child / zoom #XX
...
%=========================================================================================
%[ddd] = argument_def_dir(input_def_dir);
[define_dir]=argument_def_dir(input_def_dir);  %%->flag to define directly the orientation / direction of the runoff
%
if define_dir==1
...
%
[latriv,lonriv,my_flow,myrivername,rivernumber]=runoff_glob_extract(grdname,global_clim_rivername,str2double(MONTH));
...
if rivernumber == 0  %%at least a river
    ...
    output_indomain_last = indomain_last;
    end 
    
    if define_dir==3
    for k0=1:number_rivertoprocess
        k=rivertoprocess(k0);
        indomain_last(k)=input_indomain_last(k);
        if indomain_last(k)==0 && plotting_zoom % Close the (zoom) figure if the river is not used
            close(nfig0+k0);
        end
    end
    ...
    for k0=1:number_rivertoprocess
        ...
        jj=J(k);
        ii=I(k);
        dir(k,:)   % For some mysterious reason adding this line save us from getting an error
        dir2=dir(k,:);
        %
    ...
    if psource_ncfile_ts==1
        ...
        if psource_ncfile_ts_auto
            ...
            for k0=1:number_rivertoprocess
                ...
                T=mean(squeeze(ncclim{'temp'}(:,N,j_rho,i_rho)),1);
                my_temp_src0(k,:)=T';
                %%
                %% For salinity ... ?
                %%
                S=mean(squeeze(ncclim{'salt'}(:,N,j_rho,i_rho))-10,1);  % hum...
...
Lsrc_=mean(my_temp_src0(:,:),2);
Tsrc_=mean(my_salt_src0(:,:),2);
number_of_river = length(Tsrc_);
